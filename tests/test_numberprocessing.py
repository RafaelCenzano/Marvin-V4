...
def test_sig_fig_counter_for_three():
    valuesToTest = [265, 1250, 0.434, 0.0155, 512, 1.23,0.701, -826, -93300, -0.234, 0.903, -0.000999]
    for value in valuesToTest:
        assertEqual(physics.numberProcessing.count_sig_figs(value), 3)


def test_sig_fig_counter_for_four():
    valuesToTest = [1011, 4567, 56.71, 123.7, 1.348, 0.01234, 0.0004444, -7729, -2787000, -0.8633, -0.2703, -0.002048]
    for value in valuesToTest:
...
def test_check_value_decimal():
    valuesToTest = [0.2324, 0.0025757, 0.000000575, 0.5, -0.57, -0.0005757, -0.006]
    for value in valuesToTest:
        assertTrue(physics.numberProcessing.checkValue(value))


def test_check_value_float():
    valuesToTest = [343.23, 340305.34, 5.1, 343.0023, -232.34343, -3435.232 - 2.2424]
    for value in valuesToTest:
        assertTrue(physics.numberProcessing.checkValue(value))


def test_check_value_strings():
    valuesToTest = ['hello', 'world', 'marvin', 'this is a string', 'C', 'B', 'This should return false']
    for value in valuesToTest:
...
def test_check_value_lists():
    valuesToTest = [[5, 343], [], [23232, 23, 23, 232], ['hello', 'world'], ['Hello', 'tests']]
    for value in valuesToTest:
        assertFalse(physics.numberProcessing.checkValue(value))
...
def test_cleanup_form_float_with_zero():
    valuesToTest = ['5.0', '-6.0', '2424.0', '239239293.0', '238.0', '-2323232.0', '-7565353.0']
    for value in valuesToTest:
        assertEqual(type(physics.numberProcessing.formCleanup(value)), type(5.1))


def test_cleanup_form_float():
    valuesToTest = ['87402.2342343243', '32432432.343243243', '-34344.434', '34343.2', '-43534.76655']
    for value in valuesToTest:
        assertEqual(type(physics.numberProcessing.formCleanup(value)), type(5.1))


def test_cleanup_form_integer():
    valuesToTest = ['3434', '34', '99877574890000', '304983874839798', '-3434', '-756646464']
    for value in valuesToTest:
        assertEqual(type(physics.numberProcessing.formCleanup(value)), type(5))


def test_cleanup_form_none():
    valuesToTest = ['hello world', '5.6.5', 'ashdjs', '984876fasfas', '897haoiudh782gbaj', '-378tgybuhfjk', '353535.353535353.533', '-3343.4343.34', '-3434.dsfhdf.4343', None]
    for value in valuesToTest:
