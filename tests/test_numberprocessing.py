from . import numberProcessing, assertEqual, assertTrue, assertFalse

'''
Test Scientific Notation conversion function
'''


def test_scientific_notation_one():
    assertEqual(
        numberProcessing.scientificNotation(
            500000, 2), '5.0 * 10^5')
...
def test_scientific_notation_nine():
    assertEqual(numberProcessing.scientificNotation(5, 2), '5')
...
def test_scientific_notation_eleven():
    assertEqual(numberProcessing.scientificNotation(9999, 2), '9999')


def test_scientific_notation_thirteen():
    assertEqual(numberProcessing.scientificNotation(0.002, 2), '0.002')
...
def test_scientific_notation_sixteen():
    assertEqual(numberProcessing.scientificNotation(0, 2), '0')

def test_scientific_notation_sixteen():
    assertEqual(numberProcessing.scientificNotation(-120, 2), '-120')

def test_scientific_notation_seventeen():
    assertEqual(numberProcessing.scientificNotation(-0.000021, 2), '-2.1 * 10^-5')

def test_scientific_notation_eighteen():
    assertEqual(numberProcessing.scientificNotation(-0.000003, 1), '-3 * 10^-6')

def test_scientific_notation_nineteen():
    assertEqual(numberProcessing.scientificNotation(-3092300, 1), '-3 * 10^6')

def test_scientific_notation_twenty():
    assertEqual(numberProcessing.scientificNotation(0.0000001, 4), '1.000 * 10^-7')

def test_scientific_notation_twenty_one():
    assertEqual(numberProcessing.scientificNotation(-0.0000001, 3), '-1.00 * 10^-7')

def test_scientific_notation_twenty_two():
    assertEqual(numberProcessing.scientificNotation(-0.00000074, 3), '-7.40 * 10^-7')

def test_scientific_notation_twenty_two():
    assertEqual(numberProcessing.scientificNotation(0.00000000070095, 7), '7.009500 * 10^-10')
...
def test_sig_fig_counter_for_one():
    ...
    for value in valuesToTest:
        assertEqual(numberProcessing.count_sig_figs(value), 1)
...
def test_sig_fig_counter_for_two():
    valuesToTest = [
        ...
        '51,000']
    for value in valuesToTest:
        assertEqual(numberProcessing.count_sig_figs(value), 2)
...
def test_sig_fig_counter_for_three():
    valuesToTest = [
        ...
        '231,000']
    for value in valuesToTest:
        assertEqual(numberProcessing.count_sig_figs(value), 3)
...
def test_sig_fig_counter_for_four():
    valuesToTest = [
        ...
        '1,234,000']
    for value in valuesToTest:
        assertEqual(numberProcessing.count_sig_figs(value), 4)


def test_sig_fig_counter_for_90000():
    valuesToTest = ['9.8', '0', '-9.8']
    for value in valuesToTest:
        assertEqual(numberProcessing.count_sig_figs(value), 90000)
...
def test_proper_rounding_one_integer():
    assertEqual(
        numberProcessing.properRounding(
            345, 2), 340)
...
def test_proper_rounding_nine_integer():
    assertEqual(
        numberProcessing.properRounding(-345, 2), -340)


def test_proper_rounding_ten_integer():
    assertEqual(
        numberProcessing.properRounding(-35385, 2), -35000)


def test_proper_rounding_eleven_integer():
    assertEqual(
        numberProcessing.properRounding(-8783745, 5), -8783700)
...
def test_proper_rounding_nine_decimal():
    assertEqual(
        numberProcessing.properRounding(-0.0345, 2), -0.035)


def test_proper_rounding_ten_decimal():
    assertEqual(
        numberProcessing.properRounding(-0.35385, 2), -0.35000)


def test_proper_rounding_eleven_decimal():
    assertEqual(
        numberProcessing.properRounding(-0.0008783745, 5), -0.00087837)
...
def test_proper_rounding_nine_float():
    assertEqual(
        numberProcessing.properRounding(-2424.0345, 2), -2400)


def test_proper_rounding_ten_float():
    assertEqual(
        numberProcessing.properRounding(-72.270512, 6), -72.2705)


def test_proper_rounding_eleven_float():
    assertEqual(
        numberProcessing.properRounding(-17.6633456781205, 10), -17.66334568)
...
def test_check_value_int():
    ...
    for value in valuesToTest:
        assertTrue(numberProcessing.checkValue(value))
...
def test_check_value_strings():
    ...
    for value in valuesToTest:
        assertFalse(numberProcessing.checkValue(value))


def test_check_value_none():
    assertFalse(numberProcessing.checkValue(None))
...
def test_cleanup_num():
    ...
    for value in valuesToTest:
        assertEqual(numberProcessing.cleanValue(value), int(value))


def test_cleanup_num_none():
    assertEqual(numberProcessing.cleanValue(None), None)
...
def test_cleanup_form_float_with_zero():
    ...
    for value in valuesToTest:
        assertEqual(
            type(
                numberProcessing.formCleanup(value)),
            type(5.1))
...
def test_cleanup_form_integer():
    valuesToTest = [
        ...
        '-756646464']
    for value in valuesToTest:
        assertEqual(type(numberProcessing.formCleanup(value)), type(5))
...
def test_cleanup_form_none():
    valuesToTest = [
        ...
        None]
    for value in valuesToTest:
        assertEqual(numberProcessing.formCleanup(value), None)
